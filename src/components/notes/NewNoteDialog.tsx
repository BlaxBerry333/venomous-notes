...
/** 新建笔记弹框——填 path（如 `go/A99-new-topic`）+ 标题 + AI 提示占位（暂不持久化），调 createNote 落空模板，跳转 */
export function NewNoteDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    // aiPrompt 暂不持久化——留作后续接入 AI 时的输入点
    const result = await createNote(parts, {
      title: ttl,
      date: new Date().toISOString().slice(0, 10),
      tags: [],
    })
    if (result.ok) {
      router.push(`/notes/${parts.join('/')}`)
      onClose()
    } else {
  ...
  return (
    ...
    >
      <form
        ...
        onSubmit={submit}
      >
        <h2 className="text-fg mb-4 text-[16px] font-semibold">新建笔记</h2>
        <div className="flex flex-col gap-3">
          <label className="flex flex-col gap-1.5">
            ...
            <span className="text-muted2 text-[11px]">
              用「/」分隔，最终对应 content/notes/&lt;path&gt;.md
            </span>
          </label>
          <label className="flex flex-col gap-1.5">
          ...
          <label className="flex flex-col gap-1.5">
            <span className="text-muted text-[12px]">AI 提示</span>
            <textarea
              value={aiPrompt}
              onChange={(e) => setAiPrompt(e.target.value)}
              placeholder="告诉 AI 你想写什么主题（后续接入 AI 自动生成内容）"
              rows={3}
              className="bg-surface border-border text-fg focus:border-accent resize-none rounded-md border px-3 py-2 text-[13.5px] leading-relaxed transition outline-none"
            />
          </label>
          {error && (
